...
def action(x, current, portfolio, price):
...
for ind in range(int(n/60)*30,n):
    ...
    # Take action
    portfolio,finalPrice = action(xopt, current, portfolio, close[ind])
    coins = portfolio[0]
    cash = portfolio[1]
    coinss[ind] = coins
    cashs[ind] = cash
